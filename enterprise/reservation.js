...
function reservePlace(travel){
  ...
  if (travel.places-travel.reservedPlaces>0) {
  	...
  	console.log('[LOG] - Reservation '+ reserve.id +' performed for travel '+travel.id+' from '+travel.originCity+' to '+travel.destinyCity+'. Total reserved places: '+travel.reservedPlaces+'. Total available places: '+(travel.places-travel.reservedPlaces));
  } else {
  	reserve.status = FAILED_STATUS;
  	console.log('[LOG] - Reservation '+ reserve.id +' failed for travel '+travel.id+' from '+travel.originCity+' to '+travel.destinyCity+'. Total reserved places: '+travel.reservedPlaces+'. Total available places: '+(travel.places-travel.reservedPlaces));
  }
  return reserve;
}
...
function reservationCancellation(travel,reserveIndex,reserveId){
	setTimeout(function(){
		...
		if (reserve.id === reserveId && reserve.status === PENDING_STATUS) {
			...
			console.log('[LOG] - Reservation '+ reserve.id +' cancelled for travel '+travel.id+' from '+travel.originCity+' to '+travel.destinyCity+'. Total reserved places: '+travel.reservedPlaces+'. Total available places: '+(travel.places-travel.reservedPlaces));}
		}
